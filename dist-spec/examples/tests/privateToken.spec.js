...
import { AccountBalanceQuery, AccountCreateTransaction, AccountId, Client, Hbar, PrivateKey, TokenAssociateTransaction, TokenCreateTransaction, TokenSupplyType, TransactionId, TransactionReceiptQuery, TransferTransaction, } from '@hiero-ledger/sdk';
...
const PROTO_ROOT = path.resolve(REPO_ROOT, 'workspace', 'hiero-consensus-node', 'hapi', 'hedera-protobuf-java-api', 'src', 'main', 'proto');
const TRANSACTION_PROTO = path.join(PROTO_ROOT, 'services', 'transaction.proto');
const SWIRLDS_LOG_PATH = '/opt/hgcapp/services-hedera/HapiApp2.0/output/swirlds.log';
const DEFAULT_NODE_ID = AccountId.fromString('0.0.3');
const require = createRequire(import.meta.url);
const TokenTypeModule = require(path.join(SOLO_ROOT, 'node_modules', '@hiero-ledger', 'sdk', 'lib', 'token', 'TokenType.js'));
...
async function main() {
    ...
    console.log(`Using deployment ${solo.deploymentName} in namespace ${solo.namespace} via ${solo.grpcEndpoint}`);
    ...
    const tokenRx = await new TokenCreateTransaction()
        ...
        .setTokenType(PRIVATE_TOKEN_TYPE)
        .execute(client)
        .then((tx) => tx.getReceipt(client));
    ...
    console.log(`Crafted output commitments: recipient ${split.recipientCommitmentHex}, change ${split.changeCommitmentHex}`);
...
function ensurePrivateTokenType() {
    const current = TokenType.FungiblePrivate;
    if (current) {
        return current;
    }
    const instance = new TokenType(2);
    TokenType.FungiblePrivate = instance;
    const originalFromCode = TokenType._fromCode.bind(TokenType);
    TokenType._fromCode = (code) => (code === 2 ? instance : originalFromCode(code));
    return instance;
}
async function locateSoloDeployment() {
...
async function loadOperatorCredentials(accountsFile) {
...
async function readInitialCommitmentHex(solo, tokenId) {
    const command = `grep -F "Initial private supply" ${SWIRLDS_LOG_PATH} | tail -n 50`;
    const output = await runCmd('kubectl', [
        '-n',
        solo.namespace,
        'exec',
        solo.podName,
    ...
    throw new Error(`Unable to find commitment for ${searchToken} inside swirlds.log`);
}
function createCommitmentSplit(inputHex, transferAmount) {
...
async function submitPrivateTransfer(args) {
    ...
    const protoClient = createTransactionServiceClient(solo.grpcEndpoint);
    const response = await new Promise((resolve, reject) => {
        protoClient.submitTransaction(payload, (err, value) => {
            if (err) {
                reject(err);
            }
            else {
    ...
    if (status !== 0) {
        throw new Error(`submitTransaction returned precheck ${status}`);
    }
    protoClient.close();
...
function buildSignatureMap(pubKey, signature) {
    const root = loadTransactionProto();
...
function buildSignedTransaction(bodyBytes, sigMap) {
    const root = loadTransactionProto();
...
function buildTransactionEnvelope(signedTransactionBytes) {
    const root = loadTransactionProto();
    const Transaction = root.lookupType('proto.Transaction');
    return Transaction.create({ signedTransactionBytes });
}
function createTransactionServiceClient(endpoint) {
    const packageDefinition = protoLoader.loadSync(TRANSACTION_PROTO, {
        keepCase: true,
        longs: String,
        enums: Number,
        defaults: false,
    ...
    const TransactionService = descriptor.proto.Transaction;
    return new TransactionService(endpoint, grpc.credentials.createInsecure());
}
...
async function assertZeroBalance(client, accountId, tokenId) {
    const balance = await new AccountBalanceQuery().setAccountId(accountId).execute(client);
    const bal = balance.tokens?.get(tokenId) ?? 0;
...
async function expectPublicTransferFailure(client, operatorId, recipientId, tokenId, amount) {
    try {
...
function matchLine(source, pattern) {
